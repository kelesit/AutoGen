...
其他分支：429 明确未接单时退回排队；查单、轮询等恢复超过重试预算时也进入待核查；在轮询仍返回 running、且距任务创建已超过 3 分钟时进入人工核查；当前计时包含排队时间，并非从供应商接单开始计时。管理员可以恢复可核查任务的原阶段，不提供通用“强制再生成”按钮。
...
- 任务保存模板版本、模型名、预设提示词和报价版本快照。前端通过报价接口显示金额并提交金额/价格版本，服务端重新计算费用，并在确认字段存在时核对。API 当前允许省略确认字段，不强制先报价；扣费金额仍完全由服务端决定。
...
文件清理由 AssetService 负责。模板当前引用、未结束任务、有效作品和素材库保护文件；历史记录不永久占用文件。清理先在事务中标记 deleting，再幂等删除文件，失败可重试。详见 [媒体生命周期](docs/media-lifecycle-design.md)。
