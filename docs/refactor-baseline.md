...
3. 模板 public/private/deleted；下架停止公开使用。删除不可恢复，保留模板记录、发布请求、版本和关联证据，不裁剪为最小记录。旧版本无回滚承诺；旧版本本身不保护文件，当前模板或活动任务等有效保留根仍会保护同一资产。
4. 任务只使用提交时的版本和输入。新任务必须携带确认金额与价格版本，并与服务端重算报价一致；校验失败不受理、不冻结积分。提交、引用绑定、积分冻结、入队为同一事务；外部调用不跨数据库事务。相同请求标识与业务参数的重试直接返回原任务，不重复冻结或按当前报价重新计费。
...
10. 动作参考和示例视频在模板表单中上传，没有独立素材库或复用入口；新上传但未被引用的文件在 24 小时后由 Worker 清理。图片为临时上传，同样按占用情况清理；未清理且仍为 ready 时可绑定任务。作品封面不再依赖用户输入图。
11. 资产清理状态 ready -> deleting -> deleted。新增绑定只允许 ready，在事务中校验。删除标记只为幂等/追溯，不是回收站。
12. 记录来源/参数的历史关联不永久保留文件。未结束任务、有效作品和当前模板是保留根。
