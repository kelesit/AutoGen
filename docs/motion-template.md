...
前端通过 `POST /api/quote` 提交模板 ID、分辨率和时长，获取服务端计算的 `cost` 与 `version`，展示给用户确认。`POST /api/jobs` 创建新任务时必须携带 `expectedCost`（非负安全整数）与 `priceVersion`（非空字符串）；缺失或类型无效返回 400，与服务端重算报价不一致返回 409，要求重新确认。拒绝受理时不创建任务、不绑定素材、不冻结积分。当前校验确认字段与报价的一致性，不签发或保存必须先调用报价接口的凭证。

相同幂等键且业务参数相同的重试返回原任务，即使模板后来修改或删除也不重复生成；业务参数不同返回 409。已有任务的重试不要求补充报价字段，也不按当前报价重新计费，始终保留原任务的报价快照。
...
- 管理目录支持分页（默认每页 10，最多 100），公开目录一次返回所有上架模板。动作参考和示例视频在新建或编辑模板时上传；后台不提供独立素材库或跨模板复用入口。
- 新上传的模板视频在当前模板或未结束任务引用时保留；未引用的上传文件在宽限期后由 Worker 清理。
- `GET /api/collection` 返回最多 100 条卡片，未结束任务优先；`totalCreations`、`activeTasks` 与 `reviewTasks` 是全量计数。`activeTasks` 包含待核查任务，页面将待核查数量单独展示。已删除作品不再进入该列表；失败和取消任务仍会显示。`GET /api/jobs` 保留任务历史，最多最近 100 条。
